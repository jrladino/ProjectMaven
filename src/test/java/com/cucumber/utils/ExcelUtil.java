...
public class ExcelUtil {
  ...
  // Método que recibe la ruta de un archivo Excel, busca fila disponible
  // "Disponible"
  public static Map<String, String> obtenerRegistroDisponibleYMarcarUsado(String rutaArchivo) {
    // Mapa para almacenar los datos de la fila encontrada (clave= nombre columna,
    // valor= valor celda)
    ...
    try {
      ...
      DataFormatter formatter = new DataFormatter(); // Formateador para obtener valores como String
      ...
      // Recorre todas las filas empezando desde la segunda fila (índice 1), por que
      ...
      for (int i = 1; i <= sheet.getLastRowNum(); i++) {
        ...
        if (celdaEstado == null)
          continue; // Salta si la celda está vacía

        // Se asegura que la celda sea de tipo String
        // celdaEstado.setCellType(CellType.STRING); // No recomienda cambiar tipo de
        // celda
        String estado = formatter.formatCellValue(celdaEstado).trim(); // limpia espacios en blanco

        // Si el estado es "Disponible", lee los datos de la fila
        if (estado.equalsIgnoreCase("Disponible")) {
          // Recorre todas las celdas de la fila
          for (Cell celda : fila) {
            // celda.setCellType(CellType.STRING); // No recomienda cambiar tipo de celda
  ...
  // MEtodo para buscar solo el primer registro con estado "Disponible" y marcarlo
  // como "Usado"
